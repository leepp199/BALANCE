...
class Classifier(nn.Module):
    def __init__(self, args, feat_dim, train_weight_base=False):
        ...
        # 原有组件保持不变
        self.calibrator = SupportCalibrator(nway=args.n_ways, feat_dim=feat_dim, n_head=1, 
                                          base_seman_calib=args.base_seman_calib, neg_gen_type=args.neg_gen_type)
        self.open_generator = OpenSetGenerater(args.n_ways, feat_dim, n_head=1, 
                                             neg_gen_type=args.neg_gen_type, agg=args.agg)
        self.metric = Metric_Cosine()
        
        # ============ 新增UNCG组件 ============
        # MC Dropout层（用于不确定性估计）
        self.dropout_layers = nn.ModuleList([
            nn.Dropout(p=args.uncertainty.dropout_rate) for _ in range(3)
        ])
        
        # 不确定性估计配置
        self.uncertainty_config = {
            'num_mc_samples': getattr(args.uncertainty, 'num_mc_samples', 10),
            'num_masks': getattr(args.uncertainty, 'num_masks', 5),
            'mask_ratio': getattr(args.uncertainty, 'mask_ratio', 0.15),
            'use_nuclear_norm': getattr(args.uncertainty, 'use_nuclear_norm', True)
        }
        
        # 重要性权重缓存（用于增量学习）
        self.importance_weights_cache = None
        self.enable_uncertainty = False  # 控制开关

    def enable_uncertainty_estimation(self, enable=True):
        """启用/禁用不确定性估计"""
        self.enable_uncertainty = enable
        for layer in self.dropout_layers:
            layer.train(enable)  # 训练时启用Dropout，推理时禁用
    ...
    def estimate_uncertainty(self, features):
        ...
        # MC Dropout采样循环
        for k in range(self.uncertainty_config['num_mc_samples']):
            # 掩码增强循环
            for a in range(self.uncertainty_config['num_masks']):
                # 应用不同的Dropout和掩码模式
                processed_features = features.clone()
                
                # 应用MC Dropout
                for dropout_layer in self.dropout_layers:
                    processed_features = dropout_layer(processed_features)
                
                # 应用随机掩码
                masked_features = self.apply_random_mask(processed_features)
                
                # 获取该配置下的预测
                with torch.no_grad():
                    pred = self.standard_forward(masked_features)
                    all_predictions.append(pred)
        
        # 构建预测矩阵并计算不确定性
        prediction_matrix = torch.stack(all_predictions, dim=0)
        uncertainty = self.nuclear_norm_uncertainty(prediction_matrix)
        
        return uncertainty

    def apply_random_mask(self, features):
        """
        对特征应用随机掩码
        """
        batch_size, feat_dim = features.shape[-2], features.shape[-1]
        masked_features = features.clone()
        
        # 为每个样本生成不同的掩码模式
        for i in range(batch_size):
            # 随机选择要掩码的特征维度
            mask_indices = torch.randperm(feat_dim)[:int(feat_dim * self.uncertainty_config['mask_ratio'])]
            
            if len(features.shape) == 3:  # [batch, seq, feat]
                masked_features[i, :, mask_indices] = 0
            else:  # [batch, feat]
                masked_features[i, mask_indices] = 0
        
        return masked_features

    def nuclear_norm_uncertainty(self, prediction_matrix):
        """
        基于核范数的不确定性计算
        """
        uncertainty_scores = []
        
        for i in range(prediction_matrix.size(1)):  # 遍历每个样本
            sample_preds = prediction_matrix[:, i, :]  # [K×A, num_classes]
            
            # 奇异值分解计算核范数
            U, S, V = torch.svd(sample_preds)
            nuclear_norm = torch.sum(S)  # 核范数作为不确定性度量
            
            uncertainty_scores.append(nuclear_norm.item())
        
        return torch.tensor(uncertainty_scores, device=prediction_matrix.device)

    def calculate_importance_weights(self, uncertainty_scores):
        """
        根据不确定性计算重要性权重
        低不确定性 → 高权重（简单样本优先学习）
        """
        max_uncertainty = uncertainty_scores.max()
        weights = max_uncertainty / (uncertainty_scores + 1e-8)
        return weights / weights.max()  # 归一化到[0,1]

    def standard_forward(self, features):
        """
        标准前向传播（用于不确定性估计）
        """
        # 简化的前向传播，仅用于获取预测
...
def incremental_update_with_weights(self, features, labels, importance_weights):
        """
        重要性加权的增量学习更新
        """
        self.train()
        self.enable_uncertainty_estimation(True)
        
        # 1. 提取特征
        encoded_features = self.encode(features) if hasattr(self, 'encode') else features
        
        # 2. 计算加权损失
        logits = self.forward(encoded_features)
        loss = F.cross_entropy(logits, labels, reduction='none')
        weighted_loss = (loss * importance_weights).mean()
        
        return weighted_loss

    def cache_importance_weights(self, weights):
        """缓存重要性权重用于后续增量学习"""
        self.importance_weights_cache = weights

    def get_cached_weights(self):
        """获取缓存的重要性权重"""
        return self.importance_weights_cache