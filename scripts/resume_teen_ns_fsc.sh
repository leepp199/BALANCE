...
run_one() {
  ...
  CUDA_VISIBLE_DEVICES=0 python train_unopenset.py -config "$config" -dataset "$dataset" \
    --dataroot "$root" --checkpoint True --checkpoint_name epoch_15.pth \
    --num_labeled_classes "$base" --num_unlabeled_classes 5 \
    --opt_version sota_three_datasets --run_tag "$tag" \
    --save_result /data/lqq/baseline_dfsb/save_result/ --mixed_openworld_stream True \
    --structure_discovery_weight 0.0 --cluster_all_candidates False \
...
run_one nsynth-100 configs/exp_ns100.yml /data/datasets/The_NSynth_Dataset/ \
  80 teen_pure_ns100_10runs
run_one FMC configs/exp_fsc89.yml \
  /data/datasets/FSD-MIX-CLIPS-for_FSCIL/FSD-MIX-CLIPS_data \
  69 teen_pure_fsc89_10runs
