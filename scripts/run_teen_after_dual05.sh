...
while kill -0 2642358 2>/dev/null; do
  sleep 20
done

cd /data/lqq/baseline_dfsb
mkdir -p logs/mixed_openworld save_result/mixed_openworld
CUDA_VISIBLE_DEVICES=0 python train_unopenset.py \
  ...
  --opt_version mixed_openworld \
  --run_tag teen_a09_dual05_10runs \
  ...
  --eval_repeats 10 \
  2>&1 | tee logs/mixed_openworld/teen_a09_dual05_10runs.log
