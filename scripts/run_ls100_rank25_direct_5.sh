...
python train_unopenset.py \
  ...
  --balanced_kmeans True --balanced_kmeans_iters 5 --normalize_cluster_features True \
  --compact_steps 30 --compact_base_margin 0.2 --compact_novel_margin 0.0 \
  --stat_memory False --teen_calibration False --novel_bank_classifier False \
  --session_restricted_alignment True --reset_fc_each_round True --eval_repeats 5 \
  --run_tag ls100_rank25_direct_5 \
  2>&1 | tee logs/ls100_rank25_direct_5.log
