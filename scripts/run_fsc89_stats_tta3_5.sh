...
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-2}" python train_unopenset.py \
  ...
  --discovery_ranked_topk 25 --discovery_rank_score encode_joint_stats \
  --base_geometry_path /data/lqq/baseline/save/exp_fsc89_69supcon/base_val_geometry_features.pth \
  --discovery_encoder direct --encode_tta_views 3 \
  --normalize_cluster_features True --use_joint_cluster_assignments True \
  --joint_cluster_layer layer4 --joint_margin_weight 0.5 --joint_kmeans_trials 5 \
  --joint_kmeans_random_state seeded \
  --cluster_algorithm kmeans --prototype_trim_farthest 0 --compact_steps 0 \
  --balanced_kmeans True --incremental_metric cosine \
  --novel_bank_classifier False --session_restricted_alignment True \
  --run_tag fsc89_stats_tta3_5 \
  2>&1 | tee logs/fsc89_stats_tta3_5.log
