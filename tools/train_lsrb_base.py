#!/usr/bin/env python3
"""Train the encoder with a frozen Latent Structure Reference Bank."""
...
from models.lsrb import LatentStructureReferenceBank, MaskedStructurePredictor
...
def parse_args():
    ...
    parser.add_argument("--output", default=str(ROOT / "artifacts/checkpoints/lsrb_ls100_base.pth"))
    parser.add_argument("--history", default=str(ROOT / "artifacts/checkpoints/lsrb_ls100_history.json"))
...
def main():
    ...
    for epoch in range(cli.epochs):
        ...
        count = 0
        progress = tqdm(loader, desc=f"LSRB epoch {epoch + 1}/{cli.epochs}")
    ...
    print(f"saved best LSRB checkpoint to {output_path}")
